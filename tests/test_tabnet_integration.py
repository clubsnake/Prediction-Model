...
from src.tuning.meta_tuning import suggest_model_hyperparameters
from src.training.walk_forward import train_model_for_fold, generate_predictions, analyze_model_performance
...
def test_tabnet_training():
    ...
    y = 2*X['feature1'] + 0.5*X['feature2']**2 + np.random.normal(0, 0.5, n_samples)
...
def test_tabnet_in_ensemble():
    ...
    try:
        ...
        ensemble_preds, all_preds = generate_predictions(fold_models, X_test, ensemble_config)
        ...
        performance = analyze_model_performance(results, fold_models, X_test, feature_cols)
