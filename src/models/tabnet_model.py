    ...
    def feature_importances(self, kind: str = "gain") -> np.ndarray:
        ...
        try:
            if kind == "gain":
                ...
                raise ValueError(f"Unknown importance type: {kind}. Use 'gain' or 'mask'.")
...
class TabNetOptunaOptimizer:
    ...
    def plot_study(self, output_dir: Optional[str] = None) -> Dict[str, str]:
        ...
        try:
            ...
            importances_path = os.path.join(output_dir, f"{self.study_name}_importances.png")
            ...
            try:
                ...
                if len(top_params) >= 2:
                    fig = optuna.visualization.matplotlib.plot_contour(self.study, top_params)
                    contour_path = os.path.join(output_dir, f"{self.study_name}_contour.png")
    ...
    def combine_feature_importances(
        ...
        for model_dict in models:
            ...
            try:
                ...
                logger.warning(f"Error getting feature importances for {name}: {str(e)}")
        
        # Calculate weighted average
        if combined_importances.shape[1] > 0:
            total_weight = sum(model_dict["weight"] for model_dict in models 
                              if hasattr(model_dict["model"], "feature_importances"))
            
            if total_weight > 0:
                combined_importances["ensemble_average"] = combined_importances.sum(axis=1) / total_weight
...
def example_usage():
    ...
    X = np.random.randn(n_samples, n_features)
    y = 100 + 5 * X[:, 0] + 2 * X[:, 1] ** 2 - 3 * X[:, 2] + np.random.randn(n_samples) * 10
    feature_names = [f"feature_{i}" for i in range(n_features)]
    ...
    ensemble_mape = np.mean(np.abs((y_test - ensemble_pred) / (np.abs(y_test) + 1e-10))) * 100
