...
import src.models.nbeats_model as nbeats_model
from sklearn.ensemble import RandomForestRegressor
from src.models.temporal_fusion_transformer import TemporalFusionTransformer
from tensorflow.keras import Sequential  # type: ignore
from tensorflow.keras.layers import (LSTM, BatchNormalization,  # type: ignore
                                     Dense, Dropout, InputLayer, SimpleRNN)
...
try:
    from config.config_loader import get_value  # Import get_value to use for configuration
    ...
    LOOKBACK = 30
    import logging
    logging.warning("Could not import from config module. Using fallback LOOKBACK value.")
...
from src.training.callbacks import DynamicLearningRateScheduler
...
def build_model_by_type(model_type, num_features, horizon, learning_rate, dropout_rate, 
                        loss_function, lookback, architecture_params=None, trial=None):
    """
    ...
    if trial is not None:
        ...
        from src.utils.hyperparameter_utils import HyperparameterManager
        ...
        for param_name in ["units", "dropout", "learning_rate"]:
            suggested_params[param_name] = hyperparameter_manager.suggest_parameter(trial, f"{model_type}_{param_name}")
    ...
    if model_type == "lstm":
        ...
        model = build_nbeats_model(
            ...
            num_features=num_features,
            learning_rate=learning_rate,
            stack_types=architecture_params.get('stack_types', ['trend', 'seasonality']),
        ...
        model = build_ltc_model(
            ...
            dropout_rate=dropout_rate,
            recurrent_dropout_rate=architecture_params.get('recurrent_dropout_rate', 0.0),
        ...
        model = TabNetPricePredictor(
            ...
            optimizer_fn=lambda lr: tf.keras.optimizers.Adam(learning_rate=learning_rate),
...
def build_model_by_type(model_type, num_features, horizon, learning_rate, dropout_rate, 
    ...
    if trial is not None:
        ...
        if model_type == "lstm" or model_type == "rnn":
            learning_rate = trial.suggest_float(f"{model_type}_lr", 1e-5, 1e-2, log=True)
            ...
            use_batch_norm = trial.suggest_categorical(f"{model_type}_batch_norm", [True, False])
            l2_reg = trial.suggest_float(f"{model_type}_l2_reg", 0.0, 0.01)
            
            # Attention parameters
            use_attention = trial.suggest_categorical(f"{model_type}_use_attention", [True, False])
            attention_type = trial.suggest_categorical(f"{model_type}_attention_type", 
                                                   ["dot", "multiplicative", "additive"])
            ...
            attention_dropout = trial.suggest_float(f"{model_type}_attention_dropout", 0.0, 0.5)
            
            # Loss function suggestion
            from config.config_loader import get_value
            available_loss_fns = get_value("loss_functions.available", [
                "mean_squared_error", "mean_absolute_error", "huber_loss"
            ])
            
            loss_function = trial.suggest_categorical(f"{model_type}_loss", available_loss_fns)
            
            # Handle custom loss function parameters
            loss_params = {}
            if loss_function == "quantile_loss":
                quantile_values = get_value("loss_functions.optimization.quantile_values", [0.1, 0.5, 0.9])
                quantile = trial.suggest_categorical(f"{model_type}_quantile", quantile_values)
                ...
                scaling = trial.suggest_float(f"{model_type}_log_cosh_scaling", 0.1, 10.0, log=True)
    ...
    # If loss_function is a string, convert to actual function if needed
    if isinstance(loss_function, str):
        loss_function = get_loss_function(loss_function, architecture_params.get('loss_params', {}))
...
def register_loss_functions_with_optuna(trial, model_type, available_losses=None):
    ...
    if available_losses is None:
        available_losses = get_value("loss_functions.available", [
            "mean_squared_error", 
            "mean_absolute_error",
            "huber_loss",
...
def build_optimization_metrics(trial=None):
    ...
    default_weights = get_value("loss_functions.optimization.weighted_metrics", {
        "rmse": 1.0,
        "mape": 1.0,
        "huber": 0.8,
        "quantile": 1.0
