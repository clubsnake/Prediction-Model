...
class CNNBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, 
                 padding=1, dropout_rate=0.1, use_batchnorm=True, activation='relu'):
        """
...
class CNNPriceModel(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=1, num_conv_layers=5, num_filters=128, 
        ...
        self.conv_layers.append(
            CNNBlock(1, num_filters, kernel_size, stride, 
                     padding=kernel_size // 2, dropout_rate=dropout_rate, 
        ...
        for i in range(1, num_conv_layers):
            self.conv_layers.append(
                CNNBlock(num_filters, num_filters, kernel_size, stride, 
                         padding=kernel_size // 2, dropout_rate=dropout_rate, 
                         activation=activation)
            )
...
class CNNPricePredictor(BaseEstimator, RegressorMixin):
    ...
    """
    def __init__(self, input_dim=None, output_dim=1, num_conv_layers=3, num_filters=64, 
                 ...
                 early_stopping_patience=10, validation_split=0.1, random_state=None,
                 verbose=0):
        """
        ...
        for epoch in range(self.epochs):
            ...
            if val_loss < best_val_loss:
                ...
                self.best_model_state_ = {
                    key: value.cpu().clone() for key, value in self.model_.state_dict().items()
                }
            else:
                patience_counter += 1
    ...
    def predict(self, X):
        ...
        dataset = CNNDataset(X_scaled, np.zeros((len(X_scaled), self.output_dim)), 
                            lookback=self.lookback)
...
def optimize_cnn_model(X_train, y_train, X_val=None, y_val=None, n_trials=50, timeout=3600, 
                      study_name='cnn_optimization', storage=None, load_if_exists=True,
                      verbose=True):
    """
    ...
    def objective(trial):
        ...
        use_adaptive_pooling = trial.suggest_categorical('use_adaptive_pooling', [True, False])
    ...
    study.optimize(objective, n_trials=n_trials, timeout=timeout, 
                  n_jobs=1, show_progress_bar=verbose)
            ...
            for i in range(1, len(predictions)):
                if predictions[i].shape != output_shape:
                    try:
                        ...
                        logger.error(f"Could not reshape prediction {i} to {output_shape}")
                        raise ValueError(f"Models have incompatible output shapes: {shapes}")
...
def test_cnn_model():
    ...
    assert output.shape == (100, 1), f"Expected output shape (100, 1), got {output.shape}"
    ...
    assert predictions.shape == (100, 1), f"Expected predictions shape (100, 1), got {predictions.shape}"
