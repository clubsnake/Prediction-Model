...
class NBeatsBlock(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.theta_b_layer = layers.Dense(
            thetas_dim, activation="linear", use_bias=False, name="theta_b"
        )
        ...
        if share_thetas:
            ...
            self.theta_f_layer = layers.Dense(
                thetas_dim, activation="linear", use_bias=False, name="theta_f"
            )
    ...
    def _build_trend_basis(self):
        ...
        backcast_basis = np.zeros(
            (self.backcast_length, self.expansion_coefficient_dim)
        )
        forecast_basis = np.zeros(
            (self.forecast_length, self.expansion_coefficient_dim)
        )
    ...
    def _build_seasonality_basis(self):
        ...
        backcast_basis = np.zeros(
            (self.backcast_length, 2 * self.expansion_coefficient_dim)
        )
        forecast_basis = np.zeros(
            (self.forecast_length, 2 * self.expansion_coefficient_dim)
        )

        for i, freq in enumerate(backcast_frequencies):
            # Sine component
            backcast_basis[:, 2 * i] = np.sin(
                2 * np.pi * freq * backcast_time / self.backcast_length
            )
            # Cosine component
            backcast_basis[:, 2 * i + 1] = np.cos(
                2 * np.pi * freq * backcast_time / self.backcast_length
            )

        for i, freq in enumerate(forecast_frequencies):
            # Sine component
            forecast_basis[:, 2 * i] = np.sin(
                2 * np.pi * freq * forecast_time / self.forecast_length
            )
            # Cosine component
            forecast_basis[:, 2 * i + 1] = np.cos(
                2 * np.pi * freq * forecast_time / self.forecast_length
            )
    ...
    def call(self, inputs, training=None):
        ...
        if self.stack_type in [
            StackType.TREND,
            StackType.SEASONALITY,
            StackType.PRICE_SPECIFIC,
        ]:
        ...
        **kwargs,
        ...
        if isinstance(kernel_regularizer, dict):
            if kernel_regularizer.get("type") == "l1":
                self.kernel_regularizer = regularizers.l1(
                    kernel_regularizer.get("value", 0.01)
                )
...
def build_nbeats_model(
    ...
    """
    logger.info(
        f"Building NBEATS model with lookback={lookback}, horizon={horizon}, features={num_features}"
    )
    ...
    if kernel_regularizer_type:
        if kernel_regularizer_type == "l1":
            ...
            kernel_regularizer = regularizers.l1_l2(
                l1=kernel_regularizer_value / 2, l2=kernel_regularizer_value / 2
            )
    ...
    if use_volume_attention and use_attention:
        ...
        def volume_attention_mechanism(tensor):
            ...
            normalized_volume = volume / (
                tf.reduce_max(volume, axis=1, keepdims=True) + 1e-10
            )
    ...
    if use_attention and not use_volume_attention:
        ...
        x = layers.MultiHeadAttention(
            num_heads=4, key_dim=num_features, dropout=dropout_rate[0]
        )(x, x)
    ...
    for i, stack_type in enumerate(stack_types):
        ...
        if stack_type == StackType.TREND:
            exp_coef_dim = min(
                expansion_coefficient_dim[i], 5
            )  # Trend usually needs fewer dimensions
        elif stack_type == StackType.SEASONALITY:
            exp_coef_dim = min(
                expansion_coefficient_dim[i], 10
            )  # Seasonality may need more harmonics
    ...
    if use_ensemble_outputs:
        ...
        for i in range(ensemble_size):
            ensemble_head = layers.Dense(
                horizon, activation="linear", name=f"ensemble_head_{i}"
    ...
    if use_gradient_clipping:
        optimizer = optimizers.Adam(
            learning_rate=learning_rate, clipnorm=gradient_clip_norm
        )
