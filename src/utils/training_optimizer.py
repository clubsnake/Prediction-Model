"""
Optimizes hardware resource allocation for efficient parallel model training.
Handles memory management and resource allocation for running multiple models simultaneously.
"""
...
# Model type-specific resource requirements
...
class TrainingOptimizer:
    ...
    def __init__(self, config_path=None):
        ...
        logger.info(f"TrainingOptimizer initialized: {self.cpu_count} CPUs, "
    ...
    def _create_cpu_affinity_map(self) -> Dict[int, List[int]]:
        ...
        cores_per_group = max(1, self.cpu_count // 8)  # Divide available cores among 8 potential models
    ...
    def _auto_configure(self) -> Dict:
        ...
        if self.has_gpu:
            if self.gpu_memory_gb < 6:
                base_batch_size = 32  # For smaller GPUs
            elif self.gpu_memory_gb < 12:
                base_batch_size = 64  # For mid-range GPUs
            else:
                base_batch_size = 128  # For high-end GPUs
        else:
            # For CPU training, use smaller batches
        ...
        return {
            ...
            "gpu_memory_headroom_pct": 15,  # Keep 15% GPU memory free
    ...
    def get_model_settings(self, model_type: str, model_complexity: str = "medium") -> Dict:
        """
        Get optimized settings for a specific model type and complexity.
        ...
        Returns:
            Dictionary with optimized model settings
        """
        ...
        # Calculate batch size based on model type and hardware
        base_batch_size = self.config["base_batch_size"]
        batch_size = int(base_batch_size * profile["batch_size_factor"])
        
        # Adjust for model complexity
        if model_complexity == "large":
            batch_size = max(8, int(batch_size * 0.5))  # Smaller batches for large models
            learning_rate = 0.0005
        elif model_complexity == "small":
            batch_size = int(batch_size * 1.5)  # Larger batches for small models
            learning_rate = 0.002
        else:  # medium
            learning_rate = 0.001
            
        # Adjusted GPU memory fraction based on complexity
        ...
        # Get CPU allocation
        cpu_allocation = self._calculate_cpu_allocation(profile["cpu_weight"], model_complexity)
        
        # Create settings dictionary
        settings = {
            "batch_size": batch_size,
            "learning_rate": learning_rate,
            "epochs": 1,  # For incremental learning
            "gpu_memory_fraction": gpu_fraction if self.has_gpu else 0.0,
            "cpu_cores": cpu_allocation,
            "ram_gb": profile["ram_gb"],
            "clean_memory": model_complexity == "large",
            "mixed_precision": self.config.get("use_mixed_precision", True) if self.has_gpu else False,
            "threads": {
    ...
    def create_all_model_tasks(self, submodel_params_dict: Dict, ensemble_weights: Dict,
                              training_function: Callable) -> List[ModelTask]:
        """
        ...
        for model_type, weight in ensemble_weights.items():
            ...
            settings = self.get_model_settings(model_type, complexity)
            ...
            task = ModelTask(
                ...
                resources={
                    "gpu_fraction": settings["gpu_memory_fraction"],
    ...
    def optimize_task_groups(self, tasks: List[ModelTask]) -> List[List[ModelTask]]:
        ...
        available_cpu = self.cpu_count * (1.0 - (self.config.get("cpu_headroom_pct", 10) / 100.0))
        ...
        for task in gpu_tasks:
            ...
            if (current_gpu + task.resources["gpu_fraction"] > available_gpu or 
        ...
        if cpu_tasks:
            ...
            for task in cpu_tasks:
                if current_cpu + task.resources["cpu_cores"] > available_cpu and current_group:
                    groups.append(current_group)
                    current_group = [task]
                    current_cpu = task.resources["cpu_cores"]
                else:
    ...
    def run_all_models_parallel(self, submodel_params_dict: Dict, ensemble_weights: Dict,
        ...
        all_tasks = self.create_all_model_tasks(submodel_params_dict, ensemble_weights, training_function)
        
        # Optimize task grouping
        task_groups = self.optimize_task_groups(all_tasks)
        
        logger.info(f"Created {len(task_groups)} training groups for {len(all_tasks)} models")
        ...
        for group_idx, group in enumerate(task_groups):
            logger.info(f"Running training group {group_idx+1}/{len(task_groups)} with {len(group)} models")
            ...
            for thread in threads:
                if thread.is_alive():
                    logger.warning(f"Thread didn't complete in time for group {group_idx+1}")
    ...
    def _run_task_thread(self, task: ModelTask, results_dict: Dict):
        ...
        # Setup memory isolation if needed
        ...
        try:
            ...
            # Log completion
            logger.info(f"Completed {task.model_type} training in {runtime:.2f}s (Task {task.id})")
    ...
    def _setup_thread_environment(self, task: ModelTask):
        """Setup isolated environment for a task thread."""
        # Nothing to do for non-TensorFlow models
        if not task.config["settings"].get("is_tf_model", False):
            return
        ...
        try:
            ...
            tfsettings = task.config["settings"]
            ...
            if task.resources["gpu_fraction"] > 0 and self.has_gpu:
                ...
                if tfsettings.get("mixed_precision", False):
                    policy = tf.keras.mixed_precision.Policy("mixed_float16")
                    tf.keras.mixed_precision.set_global_policy(policy)
            
            # Set thread-specific CPU config
            tf.config.threading.set_inter_op_parallelism_threads(tfsettings["threads"]["inter_op"])
            tf.config.threading.set_intra_op_parallelism_threads(tfsettings["threads"]["intra_op"])
    ...
    def _cleanup_thread_environment(self, task: ModelTask):
        ...
        try:
            ...
            if task.config["settings"].get("is_tf_model", False):
    ...
    def _log_resource_utilization(self, label: str):
        ...
        try:
            ...
            logger.info(f"Resource utilization ({label}): "
    ...
    def get_status_for_dashboard(self) -> Dict:
        ...
        with self.resource_lock:
            return {
                ...
                "running_models": [task.model_type for task in self.active_tasks.values()],
                "task_statuses": {model_type: task.status for model_type, task 
                                  in list(self.active_tasks.items()) + list(self.completed_tasks.items())}
            }
    ...
    def get_model_config(self, model_type):
        ...
        config = {
            ...
            "gpu_memory_fraction": profile["gpu_memory_fraction"] if self.has_gpu else 0.0,
            "cpu_cores": min(cpu_cores, self.cpu_count // 2),  # Don't use more than half the cores
            "ram_gb": profile["ram_gb"],
            "mixed_precision": self.config.get("use_mixed_precision", True) if self.has_gpu else False,
            "threads": {
                "inter_op": self.config["inter_op_threads"],
...
def adjust_resources_for_imbalance(self, model_runtimes):
    """
    Dynamically adjust resources when significant training time imbalance is detected.
    ...
    if imbalance_ratio >= 3.0:
        logger.info(f"Significant training imbalance detected: {slowest_model[0]} is {imbalance_ratio:.1f}x slower")
        ...
        # Apply adjustments to the slowest model
        ...
        original_settings = {
            ...
            "batch": slow_profile.get("batch_size_factor", 1.0)
        }
        ...
        if slow_profile.get("gpu_memory_fraction", 0) > 0:
            ...
            # For neural net models, adjust batch size
            if slow_profile.get("tf_model", False):
                # Reduce batch size to allow more iterations
                new_batch = slow_profile["batch_size_factor"] / min(1.5, np.sqrt(adjustment_factor))
                slow_profile["batch_size_factor"] = max(0.3, new_batch)
        else:
        ...
        # Log the adjustments
        logger.info(
            f"Adjusted resources for {slowest_model[0]}: " 
            ...
            f"Batch: {original_settings['batch']:.1f} → {slow_profile.get('batch_size_factor', 1.0):.1f}"
        )
...
def render_optimizer_dashboard(optimizer: TrainingOptimizer):
    """Render dashboard components for the training optimizer."""
    import streamlit as st
    ...
    if hasattr(optimizer, 'completed_tasks') and optimizer.completed_tasks:
        ...
        with col2:
            avg_time = sum(task.runtime for task in optimizer.completed_tasks.values()) / len(optimizer.completed_tasks)
            st.metric("Avg Training Time", f"{avg_time:.2f}s")
        with col3:
            failures = sum(1 for task in optimizer.completed_tasks.values() if task.status == "error")
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if len(values) >= 3:
                        ...
                        col1, col2 = st.columns(2)
                        with col1:
                            st.progress(util_pct / 100, text=f"Compute: {util_pct}%")
                        with col2:
                            st.progress(mem_pct / 100, text=f"Memory: {used_mb}MB/{total_mb}MB")
            except:
...
def get_training_optimizer(config_path=None):
    """Create and return a TrainingOptimizer instance with optimal settings."""
    return TrainingOptimizer(config_path)