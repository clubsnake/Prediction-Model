...
def update_forecast_in_dashboard(models_dict, df, feature_cols, ensemble_weights):
    """
    Update forecast in dashboard with given model predictions.
    ...
    try:
        ...
        # Create ensemble model for forecasting
...
def update_forecast_in_dashboard(
    ...
    This function bridges the prediction service with the dashboard display.
    ...
    try:
        ...
        # Use the prediction service if available
        try:
            from src.dashboard.prediction_service import PredictionService
            
            # Create a prediction service instance with our model
            ...
            # Use the generate_future_forecast function imported from walk_forward
            ...
            # Fallback to direct generation if prediction_service isn't available
            from src.training.walk_forward import generate_future_forecast
...
Provides common plotting functions and visualization settings.
"""
...
try:
    from config.config_loader import get_value, PREDICTION_HORIZON
    
    # Get visualization settings from config
    VIZ_SETTINGS = {
        "lookback": get_value("dashboard.visualization.lookback", 30),
        "prediction_horizon": PREDICTION_HORIZON,
        "chart_height": get_value("dashboard.visualization.chart_height", 600),
        "color_theme": get_value("dashboard.visualization.color_theme", "default"),
        "show_volume": get_value("dashboard.visualization.show_volume", True),
        "indicators": {
            "show_ma": get_value("dashboard.visualization.indicators.show_ma", False),
            "show_bb": get_value("dashboard.visualization.indicators.show_bb", False),
            "show_rsi": get_value("dashboard.visualization.indicators.show_rsi", False),
            "show_macd": get_value("dashboard.visualization.indicators.show_macd", False)
        }
    }
except ImportError:
    logger.warning("Config not found, using default visualization settings")
...
def create_time_series_chart(df, x_col='date', y_col='Close', title=None, height=None):
    """
    ...
    if not HAS_PLOTTING_LIBS:
        logger.error("Plotting libraries not available. Cannot create chart.")
        return None
        
    if df is None or df.empty:
        logger.warning("Empty dataframe provided to create_time_series_chart")
        # Create empty figure
        fig = go.Figure()
        fig.update_layout(
    ...
    if height is None:
        height = VIZ_SETTINGS.get("chart_height", 600)
    
    colors = get_active_color_scheme()
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='lines',
        name=y_col,
        line=dict(color=colors.get("price_up"), width=2)
    ))
...
__all__ = ['VIZ_SETTINGS', 'COLOR_SCHEMES', 'get_active_color_scheme', 'create_time_series_chart']
