...
def visualize_neural_network_mpl(
    model, feature_names, max_neurons_per_layer=10, figsize=(12, 8)
):
    """
    ...
    for i, layer_info in enumerate(layers_info):
        ...
        if num_neurons > 0:
            ...
            for j in range(num_neurons):
                ...
                circle = plt.Circle(
                    (layer_x, neuron_y), 0.02, color="skyblue", fill=True
                )
                ax.add_patch(circle)
                ...
                if i > 0:
                    ...
                    for k in range(prev_neurons):
                        ...
                        ax.plot(
                            [prev_x, layer_x], [prev_y, neuron_y], "gray", alpha=0.3
                        )
...
def plot_training_history_plotly(history):
    ...
    fig.add_trace(
        go.Scatter(
            x=epochs, y=history_dict["loss"], mode="lines+markers", name="Training Loss"
        )
    )
...
def visualize_neural_network_plotly(
    model, feature_names=None, max_neurons_per_layer=10
):
    """
...
def create_sequential_visualization(model, input_shape, output_shape):
    ...
    # Add input shape
    layers.append({"name": "Input", "type": "Input", "shape": str(input_shape)})
    ...
    # Add output shape
    layers.append({"name": "Output", "type": "Output", "shape": str(output_shape)})

    return {"layers": layers, "total_params": "{:,}".format(model.count_params())}
