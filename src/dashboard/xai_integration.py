...
class XAIWrapper:
    ...
    def __init__(self, model=None, model_type: str = "tensorflow", feature_names: List[str] = None):
        """
        ...
        if HAS_XAI and model is not None:
            try:
                ...
                logger.info(f"Successfully initialized XAIExplainer for {model_type} model")
    ...
    def explain_prediction(self, X: np.ndarray, method: str = "shap") -> Dict:
        ...
        try:
            if method == "shap":
                ...
                return self.explainer.explain_with_permutation_importance(X, y_pred, plot=True)
            else:
    ...
    def get_feature_importance(self, X: np.ndarray) -> Dict[str, float]:
        ...
        try:
            return get_feature_importance(self.model, X, self.feature_names, self.model_type)
        except Exception as e:
    ...
    def create_pdp_plot(self, X: np.ndarray, feature_idx: int) -> Dict:
        ...
        try:
            return self.explainer.create_partial_dependence_plot(X, feature_idx, ice=True)
        except Exception as e:
...
def create_xai_explorer(st, model, X_sample, feature_names=None, key_prefix="xai"):
    ...
    with st.expander("🔍 Explainable AI Analysis", expanded=False):
        if not wrapper.has_explainer():
            st.warning("XAI functionality is not available. Make sure explainable_ai.py is in your path.")
            return None
        ...
        with xai_tabs[1]:
            ...
            if st.button("Generate Feature Importance", key=f"{key_prefix}_importance_btn"):
