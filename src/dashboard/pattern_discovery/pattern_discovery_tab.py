...
class PatternDiscoveryTab:
    ...
    def __init__(self, df, ensemble_weighter=None, lookback_days=180, min_occurrences=5, signal_threshold=1.5):
        """
    ...
    def render_tab(self):
        ...
        with st.expander("Pattern Discovery Settings", expanded=False):
            ...
            with col2:
                min_occurrences = st.number_input("Min Occurrences", 
                                                 min_value=3, 
                                                 max_value=50, 
            ...
            with col3:
                signal_threshold = st.number_input("Signal Threshold (Z-score)", 
                                                 min_value=1.0, 
                                                 max_value=3.0, 
    ...
    def _render_active_patterns_tab(self):
        ...
        if not active_patterns:
            st.info("No patterns are currently active. Run pattern discovery or wait for patterns to trigger.")
        ...
        active_patterns.sort(key=lambda x: abs(x.get('expected_return', 0)), reverse=True)
    ...
    def _render_pattern_library_tab(self):
        ...
        if not all_patterns:
            st.info("No patterns have been discovered yet. Run pattern discovery to find patterns.")
            return
        ...
        with col1:
            pattern_categories = list(set([p.get('category', 'Other') for p in all_patterns]))
            selected_categories = st.multiselect(
                "Pattern Categories",
                pattern_categories,
                default=pattern_categories
            )
        ...
        with col3:
            sort_options = ["Reliability", "Expected Return", "Discovery Date", "Occurrences"]
        ...
        if sort_by == "Reliability":
            filtered_patterns.sort(key=lambda x: x.get('reliability', 0), reverse=not ascending)
        elif sort_by == "Expected Return":
            filtered_patterns.sort(key=lambda x: x.get('expected_return', 0), reverse=not ascending)
        elif sort_by == "Discovery Date":
            filtered_patterns.sort(key=lambda x: x.get('discovery_date', '2023-01-01'), reverse=not ascending)
        else:  # Occurrences
            filtered_patterns.sort(key=lambda x: x.get('occurrences', 0), reverse=not ascending)
        ...
        for i, pattern in enumerate(filtered_patterns):
            with st.expander(f"{pattern.get('name', f'Pattern #{i+1}')} ({pattern.get('category', 'Other')})"):
                self._render_pattern_card(pattern, is_active=self._is_pattern_active(pattern))
    ...
    def _render_realtime_signals_tab(self):
        ...
        for pattern in active_patterns:
            signal_data.append({
                ...
                'activation_time': pattern.get('last_activation', datetime.now() - timedelta(hours=random.randint(1, 24))),
        ...
        if active_patterns:
            bullish_signals = [p for p in active_patterns if p.get('expected_return', 0) > 0]
            bearish_signals = [p for p in active_patterns if p.get('expected_return', 0) < 0]
            
            # Calculate aggregate signal strength
            bullish_strength = sum([p.get('expected_return', 0) * p.get('reliability', 0) for p in bullish_signals])
            bearish_strength = abs(sum([p.get('expected_return', 0) * p.get('reliability', 0) for p in bearish_signals]))
    ...
    def _render_pattern_analytics_tab(self):
        ...
        with analytics_tabs[0]:
            ...
            for pattern in all_patterns:
                performance_data.append({
                    ...
                    'Reliability': pattern.get('reliability', 0) * 100,  # Convert to percentage
            ...
            if performance_data:
                ...
                fig = px.scatter(
                    ...
                    labels={'Expected Return': 'Expected Return (%)', 'Reliability': 'Reliability Score (%)'}
                )
                ...
                # Add quadrant labels
                fig.add_annotation(x=5, y=75, text="High Quality Bullish", showarrow=False, font_size=14)
                fig.add_annotation(x=-5, y=75, text="High Quality Bearish", showarrow=False, font_size=14)
                fig.add_annotation(x=5, y=25, text="Low Quality Bullish", showarrow=False, font_size=14)
                fig.add_annotation(x=-5, y=25, text="Low Quality Bearish", showarrow=False, font_size=14)
        ...
        with analytics_tabs[1]:
            ...
            if category_data:
                ...
                cat_summary = cat_df.groupby('Category').agg({
                    ...
                    'Reliability': 'mean',
                    'Type': lambda x: (x == 'Bullish').mean() * 100  # Percentage bullish
                }).reset_index()
                
                # Flatten multi-level columns
                cat_summary.columns = ['Category', 'Avg Return', 'Count', 'Avg Reliability', '% Bullish']
                ...
                fig = px.bar(
                    ...
                    labels={'Count': 'Number of Patterns', 'Category': 'Pattern Category'}
                )
        ...
        with analytics_tabs[2]:
            ...
            st.info("Pattern correlation network requires activation history data. This is a placeholder visualization.")
            ...
            try:
                ...
                node_trace = go.Scatter(
                    ...
                    marker=dict(
                        ...
                        color=['green' if G.nodes[node]['return_val'] > 0 else 'red' for node in G.nodes()],
        ...
        with analytics_tabs[3]:
            ...
            if self.weighter is None:
                st.info("Connect an ensemble weighter to see pattern impact on predictions")
                ...
                pattern_impacts = [
                    ...
                    {"name": "MACD Crossover", "impact": -1.8, "start": 12, "duration": 4},
                ...
                for pattern in pattern_impacts:
                    ...
                    fig.add_shape(
                        ...
                        x0=dates[start_idx],
                        x1=dates[end_idx-1],
                        y0=min(pred_df['Base Prediction'].min(), pred_df['Pattern-Adjusted'].min()),
                        y1=max(pred_df['Base Prediction'].max(), pred_df['Pattern-Adjusted'].max()),
                    ...
                    fig.add_annotation(
                        ...
                        y=max(pred_df['Base Prediction'].max(), pred_df['Pattern-Adjusted'].max()),
                        text=pattern["name"],
                        showarrow=True,
                        arrowhead=2,
                        arrowcolor="red",
                ...
                st.info("Pattern impact analysis with the connected model would appear here")
    ...
    def _render_pattern_card(self, pattern, is_active=False):
        ...
        bg_color = "#e6ffe6" if expected_return > 0 else "#ffe6e6"  # Light green or light red
    ...
    def _render_sample_pattern(self):
        ...
        sample_pattern = {
            ...
            'timeframe': '1d',
            'occurrences': 14,
            'discovery_date': (datetime.now() - timedelta(days=45)).strftime('%Y-%m-%d'),
            'category': 'Technical Pattern',
            'historical_examples': [
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': 4.2},
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': 3.8},
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': 5.1},
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': -0.4},
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': 3.9},
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': 4.6},
                {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 'return': 2.8},
            ]
        }
    ...
    def _render_sample_signal_timeline(self):
        ...
        # Calculate strength
        bull_strength = sum([s['expected_return'] * random.uniform(0.6, 0.9) for s in bullish_signals])
        bear_strength = abs(sum([s['expected_return'] * random.uniform(0.6, 0.9) for s in bearish_signals]))
    ...
    def _render_price_pattern_visualization(self, vis_data):
        ...
        if not all(col.lower() in [c.lower() for c in price_df.columns] for col in required_cols):
        ...
        if "pattern_zones" in vis_data:
            for zone in vis_data["pattern_zones"]:
                ...
                color = zone.get("color", "rgba(255, 165, 0, 0.3)")  # Default: semi-transparent orange
    ...
    def _render_indicator_pattern_visualization(self, vis_data):
        ...
        # Add indicator
        indicator_col = vis_data.get("indicator_column", ind_df.columns[1] if len(ind_df.columns) > 1 else ind_df.columns[0])
    ...
    def _render_generic_pattern_visualization(self, pattern):
        ...
        if pattern_type == 'price' or category in ['Chart Pattern', 'Price Action']:
            ...
            base = 100
            trend = np.linspace(0, 20, 90) * (1 if pattern.get('expected_return', 0) > 0 else -1)
        ...
        elif pattern_type == 'indicator' or category in ['Technical Indicator', 'Oscillator']:
            ...
            indicator = 50 + np.sin(np.linspace(0, 6*np.pi, 90)) * 25 + np.random.randn(90) * 5
            ...
            fig.update_layout(
                title="Sample Indicator Pattern",
                height=600
            )
            ...
            price_trend = -np.linspace(0, 15, 90) if pattern.get('expected_return', 0) > 0 else np.linspace(0, 15, 90)
            price = base + price_trend + np.sin(np.linspace(0, 3*np.pi, 90)) * 10 + np.random.randn(90) * 3
            
            # Indicator diverges from price
            indicator_trend = np.linspace(0, 15, 90) if pattern.get('expected_return', 0) > 0 else -np.linspace(0, 15, 90)
            indicator = 50 + indicator_trend + np.sin(np.linspace(0, 3*np.pi, 90)) * 8 + np.random.randn(90) * 2
            ...
            price_points = sorted(price_peaks[-2:]) if len(price_peaks) >= 2 else [60, 75]
            ...
            divergence_type = "Bullish" if pattern.get('expected_return', 0) > 0 else "Bearish"
            fig.update_layout(
                title=f"Sample {divergence_type} Divergence Pattern",
                height=600
            )
            ...
            spike_location = 65
            volume_spike[spike_location-2:spike_location+3] = np.array([200000, 500000, 1000000, 400000, 200000])
    ...
    def _get_active_patterns(self):
        ...
        if not hasattr(self, '_mock_active_patterns'):
            ...
            categories = ['Price Action', 'Technical Indicator', 'Divergence Pattern', 'Volume Pattern']
            ...
            for i in range(random.randint(0, 5)):
                ...
                self._mock_active_patterns.append({
                    ...
                    'conditions': [
                        {'indicator': f'Sample Indicator {i+1}', 'operator': random.choice(['>', '<', '==', '<=', '>=']), 'value': round(random.uniform(0, 100), 1)}
                    ...
                    'discovery_date': (datetime.now() - timedelta(days=random.randint(10, 100))).strftime('%Y-%m-%d'),
                    'category': cat,
                    'type': p_type,
                    'current_signal_strength': random.uniform(1.0, 3.0),
                    'last_activation': datetime.now() - timedelta(hours=random.randint(1, 24))
                })
        ...
        if not hasattr(self, '_mock_inactive_patterns'):
            ...
            for i in range(random.randint(10, 20)):
                ...
                self._mock_inactive_patterns.append({
                    ...
                    'discovery_date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'),
                    'category': cat,
                    'type': p_type,
                    'historical_examples': [
                        {'date': (datetime.now() - timedelta(days=random.randint(10, 200))).strftime('%Y-%m-%d'), 
                         'return': expected_return * (1 + random.uniform(-0.5, 0.5))}
    ...
    def _calculate_pattern_stats(self, patterns):
        ...
        avg_return = sum(p.get('expected_return', 0) for p in patterns) / len(patterns) if patterns else 0
        avg_reliability = sum(p.get('reliability', 0) for p in patterns) / len(patterns) if patterns else 0
