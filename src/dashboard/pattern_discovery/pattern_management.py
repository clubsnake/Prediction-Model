...
class PatternManager:
    ...
    def __init__(self, patterns_dir=None, archive_dir=None):
        ...
        self.patterns_dir = patterns_dir or os.path.join(DATA_DIR, "Models", "discovered_patterns")
    ...
    def unarchive_pattern(self, pattern_id):
        ...
        try:
            with open(archive_index_path, 'r') as f:
                ...
                # Remove from index
                archive_index["patterns"] = [p for p in archive_index["patterns"] if p.get('id') != pattern_id]
    ...
    def auto_archive_patterns(self, age_threshold_days=180, performance_threshold=0.3):
        ...
        for pattern in patterns:
            ...
            if reliability < performance_threshold:
                self.archive_pattern(pattern_id, reason=f"low reliability: {reliability}")
                archived_patterns.append(pattern_id)
    ...
    def _merge_patterns(self, existing_pattern, new_pattern):
        ...
        if 'occurrences' in new_pattern and 'occurrences' in existing_pattern:
            merged['occurrences'] = max(existing_pattern['occurrences'], new_pattern['occurrences'])
        
        # Special handling for historical examples
        if 'historical_examples' in new_pattern and 'historical_examples' in existing_pattern:
            # Combine examples
            examples = existing_pattern['historical_examples'] + new_pattern['historical_examples']
            # Remove duplicates
            unique_examples = []
            seen_dates = set()
                        ...
                        if c1.get('operator') == c2.get('operator'):
                            ...
                            if v1 is not None and v2 is not None:
                                try:
                                    ...
                                    v1 = float(v1)
                                    v2 = float(v2)
                                    value_similarity = 1 - min(abs(v1 - v2) / max(abs(v1), abs(v2), 1), 1)
            ...
            if above_mask.sum() >= min_occurrences:
                ...
                if abs(max_zscore) >= min_zscore:
                    pattern = {
                        ...
                        'conditions': [
                            {'indicator': indicator, 'operator': '>', 'value': threshold}
                        ],
