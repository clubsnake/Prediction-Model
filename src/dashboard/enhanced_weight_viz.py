...
class WeightVisualizationDashboard:
    ...
    def _render_3d_weight_evolution(self):
        ...
        if (
            not hasattr(self.weighter, "historical_weights")
            or len(self.weighter.historical_weights) < 5
        ):
        ...
        for i, weights in enumerate(self.weighter.historical_weights):
            for model, weight in weights.items():
                weights_data.append({"timestep": i, "model": model, "weight": weight})
        ...
        fig = go.Figure(
            data=[
                go.Scatter3d(
                    ...
                    line=dict(color=self.model_colors.get(model, "#1f77b4"), width=2),
                )
                for model in models
            ]
        )
        ...
        fig.update_layout(
            ...
            scene=dict(
                ...
                yaxis_title="Model",
                zaxis_title="Weight",
                yaxis=dict(ticktext=list(models), tickvals=list(range(len(models)))),
            ),
            height=700,
    ...
    def _render_weight_flow_diagram(self):
        ...
        if (
            not hasattr(self.weighter, "historical_weights")
            or len(self.weighter.historical_weights) < 2
        ):
        ...
        if n_periods >= 3:
            periods = {"Initial": 0, "Middle": n_periods // 2, "Current": -1}
        else:
            periods = {"Initial": 0, "Current": -1}
            ...
            data=[
                go.Sankey(
                    ...
                    link=dict(
                        ...
                        value=value,
                        color=[
                            f"rgba{tuple(int(c[1:][i:i+2], 16) for i in (0, 2, 4)) + (0.5,)}"
        ...
        fig.update_layout(
            title="Weight Flow Between Time Periods", height=600, font=dict(size=12)
        )
    ...
    def _render_interactive_weight_tool(self):
        ...
        if total > 0:
            ...
            normalized_weights = {
                k: 1.0 / len(adjusted_weights) for k in adjusted_weights
            }
        ...
        st.dataframe(
            ...
            column_config={
                "Pct Change": st.column_config.ProgressColumn(
                    "Percent Change", format="%+.2f%%", min_value=-100, max_value=100
                )
            },
        )
    ...
    def _render_weight_network_graph(self):
        ...
        if (
            not hasattr(self.weighter, "model_correlation_matrix")
            or self.weighter.model_correlation_matrix is None
        ):
        ...
        for (model1, model2), correlation in corr_matrix.items():
            ...
            # Find node indices
            source_idx = next(
                (i for i, node in enumerate(nodes) if node["name"] == model1), None
            )
            target_idx = next(
                (i for i, node in enumerate(nodes) if node["name"] == model2), None
            )
    ...
    def _render_strategy_weights(self):
        ...
        has_strategy_weights = (
            hasattr(self.weighter, "strategy_weights")
            or hasattr(self.weighter, "meta_weights")
            or hasattr(self.weighter, "feature_importance")
        )

        if not has_strategy_weights:
            # Create mock strategy weights for demonstration
            st.info(
                "Creating sample strategy weight visualization (replace with actual strategy weights)"
            )
            ...
            for i, (category, weights) in enumerate(strategy_categories.items()):
                with category_tabs[i]:
                    ...
                    optimal_weights = {
                        k: v * (1 + 0.2 * np.random.randn()) for k, v in weights.items()
                    }
                    # Normalize
                    ...
                    formatted_df["Current"] = formatted_df["Current"].map(
                        "{:.4f}".format
                    )
                    formatted_df["Suggested"] = formatted_df["Suggested"].map(
                        "{:.4f}".format
                    )
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            "Change": st.column_config.ProgressColumn(
                                "Change", format="%+.4f", min_value=-0.5, max_value=0.5
                            ),
                        },
                        hide_index=True,
                    )
                    ...
                    # Add apply button
                    st.button(
                        f"Apply Suggested {category} Weights",
                        key=f"apply_{category}",
                        type="primary",
                    )
            ...
            if hasattr(self.weighter, "strategy_weights"):
                ...
                fig = px.pie(
                    ...
                    values="weight",
                    names="component",
                    title="Strategy Weights Distribution",
                )
