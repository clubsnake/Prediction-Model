...
def apply_preset_to_session_state(preset_key: str) -> None:
    ...
    try:
        ...
        st.session_state["epochs_multiplier"] = preset_multipliers.get(
            "epochs_multiplier", 1.0
        )
        st.session_state["complexity_multiplier"] = preset_multipliers.get(
            "complexity_multiplier", 1.0
        )

        logger.info(
            f"Applied preset {preset_key} with multipliers: {preset_multipliers}"
        )
        return True
...
def save_hyperparameter_configuration(
    config_name: str, description: str, param_ranges: Dict
) -> bool:
    """
    ...
    try:
        ...
        hyperparameter_config = {
            ...
            "multipliers": st.session_state.get(
                "tuning_multipliers",
                {
...
def load_hyperparameter_configuration(config_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        if "multipliers" in config_data:
            ...
            st.session_state["epochs_multiplier"] = config_data["multipliers"].get(
                "epochs_multiplier", 1.0
            )
            st.session_state["complexity_multiplier"] = config_data["multipliers"].get(
                "complexity_multiplier", 1.0
            )
