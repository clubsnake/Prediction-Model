...
def calculate_volatility(
    ...
    return returns.rolling(window=window).std().mean() * np.sqrt(
        252
...
def adaptive_window_size(
    ...
    try:
        ...
        logger.info(
            f"Adaptive window size: {window_size} (volatility: {volatility:.4f})"
        )
        return window_size

    except Exception as e:
        logger.warning(
            f"Error calculating adaptive window size: {e}. Using base_window {base_window}."
        )
        return base_window


def adaptive_retraining_threshold(
    ...
    try:
        ...
        logger.warning(
            f"Error calculating adaptive retraining threshold: {e}. Using base_threshold {base_threshold}."
        )
...
def adaptive_learning_rate(
    ...
    try:
        ...
        if new_lr != current_lr:
            logger.info(
                f"Adaptive learning rate adjusted: {current_lr:.2e} → {new_lr:.2e}"
            )
        ...
        logger.warning(
            f"Error adjusting learning rate: {e}. Keeping current_lr {current_lr}."
        )
        return current_lr
