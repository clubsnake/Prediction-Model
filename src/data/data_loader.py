...
def validate_file_exists(file_path: str, description: str = "data") -> None:
    ...
    if not os.path.isfile(file_path):
        raise DataFileNotFoundError(f"Path exists but is not a file: {file_path}")

    # Additional check for zero-size files
    if os.path.getsize(file_path) == 0:
        raise DataFileNotFoundError(f"File exists but is empty: {file_path}")
...
def load_csv_data(file_path: str, **kwargs) -> pd.DataFrame:
    ...
    try:
        ...
        raise DataFormatError(
            f"Unexpected error loading CSV file {file_path}: {str(e)}"
        )
...
def load_yaml_data(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(data, dict):
            raise DataFormatError(
                f"YAML file does not contain a dictionary: {file_path}"
            )
        ...
        raise DataFormatError(
            f"Unexpected error loading YAML file {file_path}: {str(e)}"
        )


# Add memory-efficient option to load_financial_data
def load_financial_data(
    ticker: str, timeframe: str, data_dir: str, memory_efficient: bool = True
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if isinstance(e, DataFormatError):
            raise
        raise DataFormatError(
            f"Unexpected error loading data file {file_path}: {str(e)}"
        )


def save_financial_data(
    df: pd.DataFrame, ticker: str, timeframe: str, data_dir: str
) -> str:
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully saved data for {ticker} ({timeframe}) to {file_path}"
        )
        return file_path
...
if __name__ == "__main__":
    try:
        ...
        config_file = os.path.join(
            os.path.dirname(__file__), "..", "..", "config", "user_config.yaml"
        )
