...
try:
    ...
    print("Warning: Numba not available. Using fallback implementations for optimized functions.")
...
def batch_sequence_creation(data, lookback, horizon, batch_size=1000, target_col=None):
    ...
    if num_sequences <= batch_size:
        return vectorized_sequence_creation(data, lookback, horizon, target_col=target_col)
    ...
    for start_idx in range(0, num_sequences, batch_size):
        ...
        if hasattr(data, 'iloc'):
            # For DataFrame
            batch_data = data.iloc[start_idx:start_idx + batch_length + lookback + horizon - 1]
        else:
            # For numpy arrays
            batch_data = data[start_idx:start_idx + batch_length + lookback + horizon - 1]
