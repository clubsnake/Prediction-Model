...
try:
    from config.config_loader import (
        LOOKBACK,  # Corrected import path
        PREDICTION_HORIZON,
    )
except ImportError:
    ...
    print(
        "Warning: Could not import config values. Using defaults: "
        f"LOOKBACK={LOOKBACK}, PREDICTION_HORIZON={PREDICTION_HORIZON}"
    )

# Fix the import for vectorized operations - add error handling
try:
    ...
    def vectorized_sequence_creation(df, feature_cols, target_col, lookback, horizon):
        print(
            "Warning: vectorized_sequence_creation not available - using empty implementation"
        )
        import numpy as np
    ...
    def batch_sequence_creation(
        df, feature_cols, target_col, lookback, horizon, batch_size
    ):
        print(
            "Warning: batch_sequence_creation not available - using empty implementation"
        )
        import numpy as np
    ...
    def log_memory_usage(tag):
        print(f"Warning: log_memory_usage not available - {tag}")
...
def ensure_datetime_index(df, date_col="date"):
    ...
    try:
        ...
        result_df[date_col] = pd.date_range(
            start=start_date, periods=len(result_df), freq="D"
        )
    ...
    try:
        ...
        if not use_batched or len(df) <= batch_size:
            X, y = vectorized_sequence_creation(
                df, feature_cols, target_col, lookback, horizon
            )
        else:
            # For very large datasets, use the batch processing approach
            X, y = batch_sequence_creation(
                df, feature_cols, target_col, lookback, horizon, batch_size
            )
