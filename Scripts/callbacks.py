    ...
    def on_epoch_end(self, epoch, logs=None):
        try:
            ...
            if np.isnan(current_loss) or np.isinf(current_loss):
                logger.warning(f"Skipping LR adjustment due to invalid loss: {current_loss}")
                return
...
class WarmupCosineDecayScheduler(tf.keras.callbacks.Callback):
    ...
    """
    def __init__(self, start_lr=1e-6, max_lr=1e-3, min_lr=1e-6, warmup_epochs=5, 
                 total_epochs=50, verbose=0):
...
class CyclicalLearningRate(tf.keras.callbacks.Callback):
    ...
    def __init__(self, base_lr=1e-4, max_lr=1e-2, step_size=2000, mode='triangular',
        ...
        if mode not in ['triangular', 'triangular2', 'exp_range']:
            raise ValueError(f"mode must be one of 'triangular', 'triangular2', or 'exp_range', got {mode}")
...
class AdaptiveBatchNormScheduler(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        new_momentum = self.start_momentum - progress * (self.start_momentum - self.end_momentum)
        for layer in self.model.layers:
...
class AdaptiveWeightDecay(tf.keras.callbacks.Callback):
    ...
    """
    def __init__(self, initial_decay=1e-5, min_decay=1e-7, max_decay=1e-3, 
                 patience=3, factor=2.0, monitor='val_loss'):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current_loss < self.best_loss * 0.995:
            ...
            self.wait += 1
            if self.wait >= self.patience:
                self.current_decay = min(self.max_decay, self.current_decay * self.factor)
                self.wait = 0
    ...
    def _update_weight_decay(self):
        if hasattr(self.model.optimizer, 'weight_decay'):
            if isinstance(self.model.optimizer.weight_decay, tf.Variable):
                tf.keras.backend.set_value(self.model.optimizer.weight_decay, self.current_decay)
            else:
...
class EarlyStoppingWithRestore(tf.keras.callbacks.Callback):
    ...
    """
    def __init__(self, monitor='val_loss', min_delta=0, patience=10, verbose=1, mode='auto', 
    ...
    def on_train_end(self, logs=None):
        if self.stopped_epoch > 0 and self.verbose > 0:
            ...
            if self.restore_best_weights and self.best_weights is not None:
                print(f"Model weights restored to best found at epoch {self.best_epoch + 1}")


# Example of how to use these callbacks in a TensorFlow model
def get_advanced_callbacks(learning_rate=0.001, epochs=50, batch_size=32, steps_per_epoch=100):
    """
