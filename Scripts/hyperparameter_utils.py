                            ...
                            if isinstance(node, ast.Assign):
                                for target in node.targets:
                                    if isinstance(target, ast.Name):
                                        if (isinstance(node.value, ast.Num) or 
                                            isinstance(node.value, ast.NameConstant)):
                                            param_name = target.id
                                            if param_name.isupper():  # Likely a constant
                                                parameters.append({
                                                    "file": file,
                                                    "name": param_name,
...
def suggest_parameter_registration():
    ...
    for param in params:
        ...
        print(f"{param['name']} = register_param(\"{param['name'].lower()}\", {value}, \"{param_type}\", {range_values})")
...
class HyperparameterManager:
    ...
    def suggest_all(self, trial):
        ...
        for name, config in self.registry.items():
            ...
            if param_type == "int":
                if range_values:
                    self.current_values[name] = trial.suggest_int(name, range_values[0], range_values[1])
                else:
                    # Default range based on value
                    self.current_values[name] = trial.suggest_int(name, max(1, default // 2), default * 2)
            
            elif param_type == "float":
                if range_values:
                    self.current_values[name] = trial.suggest_float(name, range_values[0], range_values[1])
                else:
                    # Default range with log scale for small values
                    if default < 0.01:
                        self.current_values[name] = trial.suggest_float(name, default / 10, default * 10, log=True)
                    else:
                        self.current_values[name] = trial.suggest_float(name, max(0.0001, default / 2), default * 2)
            
            elif param_type == "categorical":
                if range_values:
                    self.current_values[name] = trial.suggest_categorical(name, range_values)
                else:
