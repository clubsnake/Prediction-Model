...
def clean_memory(force_gc=False, clear_tf_session=True, release_gpu_memory=True, 
                verbose=False, collect_generations=2):
...
class MemoryManager:
        ...
        if self.gpu_backend == "cuda":
            try:
                ...
                if gpus:
                    ...
                    gpu_idx = 0
                    try:
                        memory_info = tf.config.experimental.get_memory_info(f'/device:GPU:{gpu_idx}')
                        self.last_gpu_memory = memory_info['current'] / (1024 * 1024)  # MB
                    except:
    ...
    def clean_memory(self, level=MemoryCleanupLevel.MEDIUM, force_gc=True):
        ...
        if level in [MemoryCleanupLevel.HEAVY, MemoryCleanupLevel.CRITICAL]:
            ...
            if current_time - self.last_cleanup_time < 30 and self.last_cleanup_level == level:
                return {
                    "success": False,
                    "reason": f"Too frequent {level} cleanup"
                }
        ...
        if level in [MemoryCleanupLevel.MEDIUM, MemoryCleanupLevel.HEAVY, MemoryCleanupLevel.CRITICAL]:
            ...
            if level == MemoryCleanupLevel.CRITICAL:
                ...
                for var_name in list(globals().keys()):
                    ...
                    if isinstance(var, np.ndarray) and var.size > 1000000:  # Arrays larger than ~8MB (assuming float64)
    ...
    def optimize_batch_size(self, model, sample_input_shape, min_batch=1, max_batch=256, safety_factor=0.8):
        """
...
def adaptive_memory_clean(level="medium"):
    ...
    try:
        ...
        if level == "large":
            ...
            process = psutil.Process(os.getpid())
            logger.info(f"Memory usage before cleanup: {process.memory_info().rss / 1e6:.1f} MB")
            ...
            logger.info(f"Memory usage after cleanup: {process.memory_info().rss / 1e6:.1f} MB")
