...
from Scripts.optuna_logger import OptunaLogger
from Scripts.walk_forward import run_walk_forward_ensemble_eval
...
class OptimizationHandler:
    ...
    def __init__(self, df, feature_cols, target_col, config=None):
        ...
        self.study_name = self.config.get('study_name', f"study_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        
        # Storage settings
        self.db_dir = self.config.get('db_dir', os.path.join("Data", "DB"))
