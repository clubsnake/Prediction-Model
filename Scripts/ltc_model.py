...
class LTCCell(tf.keras.layers.AbstractRNNCell):
    ...
    def build(self, input_shape):
        ...
        self.tau = self.add_weight(
            ...
            constraint=lambda x: tf.clip_by_value(x, self.timescale_min, self.timescale_max)
        )
    ...
    def call(self, inputs, states, training=None):
        ...
        if (training is not None) and (self.recurrent_dropout > 0):
            if self.recurrent_dropout_mask is None:
                self.recurrent_dropout_mask = self._generate_dropout_mask(h_prev, training)
            h_prev_dropped = h_prev * self.recurrent_dropout_mask
        else:
            h_prev_dropped = h_prev
        
        # Calculate update gate
        z = tf.matmul(inputs, self.kernel_z) + tf.matmul(h_prev_dropped, self.recurrent_kernel_z)
        if self.use_bias:
            z = tf.nn.bias_add(z, self.bias_z)
        z = self.recurrent_activation(z)
        
        # Calculate candidate state
        h_candidate = tf.matmul(inputs, self.kernel_h) + tf.matmul(h_prev_dropped, self.recurrent_kernel_h)
        if self.use_bias:
    ...
    def get_config(self):
        ...
        config = {
            ...
            'timescale_initializer': tf.keras.initializers.serialize(self.timescale_initializer),
            'kernel_regularizer': tf.keras.regularizers.serialize(self.kernel_regularizer),
            'recurrent_regularizer': tf.keras.regularizers.serialize(self.recurrent_regularizer),
            'bias_regularizer': tf.keras.regularizers.serialize(self.bias_regularizer),
            'timescale_regularizer': tf.keras.regularizers.serialize(self.timescale_regularizer),
...
class LTCAttentionCell(LTCCell):
    ...
    def call(self, inputs, states, training=None):
        ...
        attention = tf.matmul(inputs, self.attention_w) + tf.matmul(h_prev, self.attention_u)
        if self.use_bias:
...
def build_ltc_model(num_features: int, 
                   horizon: int, 
                   ...
                   timescale_min: float = 0.1,
                   timescale_max: float = 10.0) -> tf.keras.Model:
    """
...
def create_ltc_ensemble(num_features: int, 
                       horizon: int, 
                       learning_rate: float = 0.001,
                       loss_function: str = "mse",
                       lookback: int = 30,
                       configs: List[Dict[str, Any]] = None) -> List[tf.keras.Model]:
    """
