...
from config import (
    ...
    MODEL_TYPES, ACTIVE_MODEL_TYPES, LOSS_FUNCTIONS
)
from Scripts.model import build_model_by_type, record_weight_snapshot
...
from Scripts.vectorized_ops import (
...
def get_ensemble_model(model_types, models_dict, ensemble_weights):
    ...
    class EnsembleModel:
        def __init__(self, models_dict, weights, model_types):
            ...
            logger.info(f"Created ensemble model with {len(models_dict)} models and types: {list(models_dict.keys())}")
...
def unified_walk_forward(
    ...
    for mtype, weight in ensemble_weights.items():
        ...
        try:
            if mtype in ["lstm", "rnn"]:
                ...
                arch_params = {"units_per_layer": submodel_params_dict[mtype].get("units_per_layer", [64, 32])}
                model = build_model_by_type(
                    ...
                    loss_function=submodel_params_dict[mtype].get("loss_function", "mean_squared_error"),
                    lookback=lookback,
                ...
                from temporal_fusion_transformer import build_tft_model
    ...
    ensemble_model = get_ensemble_model(ACTIVE_MODEL_TYPES, models_dict, ensemble_weights)
    ...
    while current_idx + horizon < len(df):
        try:
            ...
            if cycle % 5 == 0 or cycle == total_cycles - 1:
                logger.info(f"Walk-forward cycle {cycle+1}/{total_cycles} (idx={current_idx}/{len(df)})")
                ...
                if cycle % 10 == 0:
                    for mtype, model in models_dict.items():
                        if mtype in ["lstm", "rnn", "tft"] and model is not None:
                            try:
                                record_weight_snapshot(model, len(st.session_state["weight_history"]))
                            except Exception as e:
                                logger.error(f"Error recording weight snapshot: {e}")
            else:
                logger.warning(f"Could not create ensemble prediction for cycle {cycle}")
                    ...
                    for mtype in ["lstm", "rnn", "tft"]:
                        if mtype in models_dict and models_dict[mtype] is None:
                            ...
                            models_dict[mtype] = build_model_by_type(
                                ...
                                loss_function=submodel_params_dict[mtype].get("loss_function", "mean_squared_error"),
                                lookback=lookback,
                                architecture_params={"units_per_layer": submodel_params_dict[mtype].get("units_per_layer", [64, 32])}
                            )
...
def calculate_mse(predictions, actuals):
    ...
    if isinstance(predictions, list) and isinstance(actuals, list):
        ...
        try:
            ...
            logger.error("Error stacking arrays for MSE calculation - shapes may be inconsistent")
...
def calculate_mape(predictions, actuals):
    ...
    epsilon = 1e-10
    return 100.0 * np.mean(np.abs((actuals.flatten() - predictions.flatten()) / (actuals.flatten() + epsilon)))
...
def run_walk_forward(model, df, feature_cols, target_col, lookback, horizon, window_size=None):
    ...
    logger.warning("Using legacy run_walk_forward - consider using unified_walk_forward instead")
    ...
    if hasattr(model, '__class__') and hasattr(model.__class__, '__name__'):
        ...
        ensemble_model = get_ensemble_model(MODEL_TYPES, models_dict, ensemble_weights)
...
# This is kept only for backward compatibility
def run_walk_forward_realistic(df, feature_cols, target_col, lookback=30, horizon=7, window_size=7):
    """Legacy function maintained for compatibility - redirects to unified implementation"""
    logger.warning("Using legacy run_walk_forward_realistic - consider using unified_walk_forward instead")
    ...
    if hasattr(ensemble_model, 'models') and "lstm" in ensemble_model.models:
        ...
        model = build_model_by_type("lstm", num_features=len(feature_cols), horizon=horizon)
