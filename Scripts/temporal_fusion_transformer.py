...
from tensorflow.keras.layers import ( # type: ignore
    Dense, LayerNormalization, Dropout, MultiHeadAttention, 
    Concatenate, Input, Embedding, GlobalAveragePooling1D, Conv1D
)
...
class PositionalEncoding(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        pos_encoding = tf.tensor_scatter_nd_update(
            pos_encoding,
            tf.stack([tf.repeat(tf.range(actual_max_pos), len(indices)), 
                     tf.tile(indices, [actual_max_pos])], axis=1),
        ...
        pos_encoding = tf.tensor_scatter_nd_update(
            pos_encoding,
            tf.stack([tf.repeat(tf.range(actual_max_pos), len(indices)), 
                     tf.tile(indices + 1, [actual_max_pos])], axis=1),
...
class VariableSelectionNetwork(tf.keras.layers.Layer):
    ...
    def __init__(self, num_features, hidden_size=64, dropout_rate=0.1, **kwargs):
        ...
        # Layers
        self.grn_layers = [GatedResidualNetwork(hidden_size, dropout_rate) for _ in range(num_features)]
...
class GatedResidualNetwork(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, hidden_size, dropout_rate=0.1, use_time_distributed=False, **kwargs):
        ...
        if use_time_distributed:
            self.dense1 = tf.keras.layers.TimeDistributed(Dense(hidden_size, activation=None))
            self.dense2 = tf.keras.layers.TimeDistributed(Dense(hidden_size, activation=None))
            self.dense3 = tf.keras.layers.TimeDistributed(Dense(hidden_size, activation=None))
            self.gate = tf.keras.layers.TimeDistributed(Dense(hidden_size, activation='sigmoid'))
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.encoder_lstm = tf.keras.layers.LSTM(
            lstm_units, return_sequences=True, activation='tanh',
...
def build_tft_model(num_features, horizon, learning_rate=0.001, hidden_size=64, 
                   lstm_units=128, num_heads=4, dropout_rate=0.1, loss_function="mse"):
    """
    ...
    sample_input = tf.random.normal((1, 30, num_features))  # Batch size of 1, sequence length of 30
    model(sample_input)
...
def add_tft_to_model_types():
    ...
    try:
        ...
        def extended_build_model_by_type(model_type, num_features, horizon, learning_rate, 
                                        dropout_rate, loss_function, lookback, architecture_params=None):
            """
            ...
            if model_type.lower() == 'tft':
                ...
                return original_build_model(
                    model_type, num_features, horizon, learning_rate, 
...
def add_tft_to_optuna_search(meta_tuning_module):
    ...
    if not hasattr(meta_tuning_module, 'ensemble_with_walkforward_objective'):
        print("Warning: meta_tuning_module does not have ensemble_with_walkforward_objective")
        return
    ...
    def extended_objective(trial, ticker, timeframe, range_cat):
        ...
        if 'tft' in meta_tuning_module.MODEL_TYPES:
            ...
            loss_functions = getattr(meta_tuning_module, 'LOSS_FUNCTIONS', ['mae', 'mse'])
            tft_loss_fn = trial.suggest_categorical(
                "tft_loss_function", loss_functions
            )
            ...
            tft_num_heads = trial.suggest_categorical(
                "tft_num_heads", [1, 2, 4, 8]
            )
            tft_dropout = trial.suggest_float(
                "tft_dropout", 0.0, 0.5
            )
            
            # Add TFT learning rate
            tft_lr = trial.suggest_float(
                "tft_lr", 1e-5, 1e-2, log=True
            )
...
if __name__ == "__main__":
    ...
    # Test predictions
    x = tf.random.normal((32, 30, 10))  # Batch size of 32, sequence length of 30, 10 features
    y = model(x)
