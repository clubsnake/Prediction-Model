# Add this to a new file Scripts/feature_selection.py
...
class FeatureSelector:
    ...
    """
    def __init__(self, df=None, target_col='Close', feature_cols=None, 
                 min_features=5, max_features=None, verbose=True):
        """
    ...
    def _prepare_data(self):
        ...
        if self.verbose:
            print(f"Data prepared with {len(self.feature_cols)} features and {len(self.y)} samples")
    ...
    def filter_by_variance(self, threshold=0.01):
        ...
        if self.verbose:
            print(f"Selected {len(selected_features)} features using variance threshold")
            print(f"Time taken: {self.results['variance_threshold']['time_taken']:.2f} seconds")
    ...
    def filter_by_correlation(self, threshold=0.95, method='pearson'):
        ...
        """
        if self.verbose:
            print(f"Filtering highly correlated features (threshold={threshold}, method={method})...")
            
        start_time = time.time()
        
        # Calculate correlation matrix
        corr_matrix = pd.DataFrame(self.X, columns=self.feature_cols).corr(method=method).abs()
        ...
        if self.verbose:
            print(f"Selected {len(selected_features)} features after dropping {len(to_drop)} highly correlated features")
            print(f"Time taken: {self.results['correlation_filter']['time_taken']:.2f} seconds")
    ...
    def select_by_mutual_info(self, k='auto', strategy='percentile', percentile=80):
        ...
        if k == 'auto':
            if strategy == 'percentile':
                ...
                k = max(self.min_features, int(len(self.feature_cols) * percentile / 100))
            else:
        ...
        if self.verbose:
            ...
            print(f"Time taken: {self.results['mutual_info']['time_taken']:.2f} seconds")
    ...
    def select_by_model(self, model_type='rf', threshold='mean', max_features=None):
        ...
        importances = selector.estimator_.feature_importances_ if hasattr(selector.estimator_, 'feature_importances_') else None
        
        if importances is not None:
            feature_importances = [(f, i) for f, i in zip(self.feature_cols, importances)]
        ...
        if self.verbose:
            ...
            print(f"Time taken: {self.results[f'model_{model_type}']['time_taken']:.2f} seconds")
    ...
    def select_by_rfe(self, model_type='rf', n_features=None, step=1, cv=None):
        ...
        """
        if self.verbose:
            print(f"Selecting features using {'CV-' if cv else ''}RFE with {model_type.upper()}...")
    ...
    def find_optimal_feature_set(self, model_type='rf', max_subset_size=20, method='add'):
        """
        ...
        # Backward elimination
            ...
            while len(selected_features) > self.min_features:
                ...
                if self.verbose:
                    print(f"  Removed feature: {worst_feature}, MSE: {best_new_score:.4f}")
        ...
        if self.verbose:
            print(f"Selected {len(best_features)} features with best MSE: {best_score:.4f}")
            print(f"Time taken: {self.results[method_key]['time_taken']:.2f} seconds")
    ...
    def combine_by_voting(self, methods=None, min_votes=2):
        ...
        for feature in self.feature_cols:
            ...
            for method in methods:
                if method in self.selected_features and feature in self.selected_features[method]:
                    votes[feature] += 1
        ...
        if self.verbose:
            print(f"Selected {len(selected_features)} features by voting (min votes: {min_votes})")
    ...
    def visualize_results(self, save_dir=None):
        ...
        if 'voting' in self.results and 'feature_votes' in self.results['voting']:
            ...
            ax.axvline(x=self.results['voting']['min_votes'], 
                       color='r', linestyle='--', 
        ...
        for method, result in self.results.items():
            if 'performance_history' in result and result['performance_history']:
                ...
                ax.set_ylabel('MSE')
                ax.set_title(f'Performance vs Number of Features ({result.get("method", method)})')
                ax.grid(linestyle='--', alpha=0.7)
                
                # Mark the best point
                best_idx = mse.index(min(mse))
                ax.plot(n_features[best_idx], mse[best_idx], 'ro', markersize=8, 
    ...
    def generate_report(self, save_path=None):
        ...
        if 'voting' in self.selected_features:
            report.append(f"Recommended features (voting): {len(self.selected_features['voting'])}")
        report.append("")
        ...
        report.append("-" * 80)
        for method, result in self.results.items():
            report.append(f"{result.get('method', method)}: "
        ...
        if 'voting' in self.results and 'feature_votes' in self.results['voting']:
            ...
            report.append("-" * 80)
            for i, (feature, votes) in enumerate(self.results['voting']['feature_votes'][:20]):
        ...
        for method, result in self.results.items():
            ...
            if 'feature_importances' in result and result['feature_importances']:
                ...
                for i, (feature, importance) in enumerate(result['feature_importances'][:10]):
            ...
            if 'performance_history' in result and result['performance_history']:
                ...
                report.append(f"    Best performance: {best_n} features, MSE={best_mse:.4f}")
        ...
        report.append("-" * 80)
        if 'voting' in self.selected_features:
            report.append(f"Recommended features (voting, {len(self.selected_features['voting'])} features):")
            for feature in self.selected_features['voting']:
                report.append(f"  - {feature}")
        else:
            # Use the first available method
            for method, features in self.selected_features.items():
                report.append(f"Recommended features ({method}, {len(features)} features):")
...
def select_optimal_features(df, target_col='Close', feature_cols=None, 
                            min_features=10, quick=True, save_dir=None):
    """
    ...
    if save_dir:
        ...
        selector.generate_report(save_path=os.path.join(save_dir, 'feature_selection_report.txt'))
...
if __name__ == "__main__":
    ...
    ticker = "ETH-USD"
    df = fetch_data(ticker, start="2023-01-01")
    df = feature_engineering(df)
    
    # Drop date column and other non-feature columns
    ...
    selected_features = select_optimal_features(
        ...
        min_features=10,
        quick=False,
        save_dir='feature_selection_results'
    )
