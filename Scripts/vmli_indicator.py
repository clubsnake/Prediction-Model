...
class VMILIndicator:
    ...
    def compute(
        ...
        if volume_col in df.columns:
            ...
            liquidity = pd.Series(1.0, index=df.index)  # Default to 1.0 if no volume data
    ...
    def _calculate_volatility(self, price_data: pd.Series) -> pd.Series:
        ...
        min_vol = volatility[volatility > 0].min() if len(volatility[volatility > 0]) > 0 else 0.0001
    ...
    def _calculate_liquidity(
        ...
        vol_factor = volume_data / vol_median.replace(0, np.nan)  # Avoid division by zero
        ...
        if order_book is not None and 'bid_volume' in order_book.columns and 'ask_volume' in order_book.columns:
            # Calculate order book imbalance
            obi = (order_book['bid_volume'] - order_book['ask_volume']) / (order_book['bid_volume'] + order_book['ask_volume'])
    ...
    def backtest(
        ...
        sortino_ratio = (excess_returns.mean() * 252) / downside_volatility if downside_volatility > 0 else 0
        ...
        win_rate = (results['trade_profit'] > 0).sum() / num_trades if num_trades > 0 else 0
