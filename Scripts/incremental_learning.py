...
from tensorflow.keras.callbacks import ModelCheckpoint, EarlyStopping, CSVLogger, ReduceLROnPlateau # type: ignore
import threading
...
class ModelRegistry:
    ...
    def __init__(self, registry_dir="model_registry", 
                 create_if_missing=True, max_models_per_type=10):
        """
    ...
    def register_model(self, model, model_type="tensorflow", 
                      ticker=None, timeframe=None, metrics=None, 
                      hyperparams=None, parent_id=None, tags=None, 
                      description=None, save_format="h5"):
        """
    ...
    def get_best_model(self, metric="rmse", ascending=True, filter_tags=None, ticker=None):
        """
        ...
        if not filtered_models:
            self.logger.warning(f"No models match the criteria (metric={metric}, tags={filter_tags}, ticker={ticker})")
            return None
        ...
        self.logger.info(f"Best model for metric {metric}: {best_model_id} with value {filtered_models[best_model_id]}")
    ...
    def _prune_old_models(self, model_type):
        ...
        models = [(model_id, meta) for model_id, meta in self.model_index.items() 
    ...
    def visualize_metrics_history(self, model_ids=None, metric="rmse", 
        ...
        if not model_ids:
            model_ids = [mid for mid, meta in self.model_index.items()
                        if meta.get("status") == "active"]
            
        # Filter models that have this metric
        valid_models = []
        for mid in model_ids:
            if mid in self.model_index and metric in self.model_index[mid].get("metrics", {}):
        ...
        legend_elements = [
            Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='TensorFlow'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='green', markersize=10, label='scikit-learn'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='XGBoost'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor='purple', markersize=10, label='Ensemble')
        ]
...
class IncrementalLearner:
    ...
    def cache_training_data(self, ticker, timeframe, df, version=None):
        ...
        self.logger.info(f"Cached training data for {ticker} ({timeframe}) version {version}")
        return version
    ...
    def get_cached_data(self, ticker, timeframe, version=None):
            ...
            if cache_key in self.data_versions and self.data_versions[cache_key]:
                ...
                for filename in os.listdir(ticker_dir):
                    if filename.startswith(f"{timeframe}_v") and filename.endswith(".pkl"):
                        versions.append(filename.replace(f"{timeframe}_v", "").replace(".pkl", ""))
                        
                if versions:
                    versions.sort()
                    version = versions[-1]
                else:
                    self.logger.warning(f"No cached data found for {ticker} ({timeframe})")
                    return None
        ...
        try:
            ...
            self.logger.info(f"Loaded cached data for {ticker} ({timeframe}) version {version}")
            return df
        except Exception as e:
            self.logger.error(f"Error loading cached data: {e}")
            return None
            
    def train_incrementally(self, model, df, ticker, timeframe, 
                           previous_model_id=None, batch_size=None, 
                           epochs=10, validation_split=0.2,
                           callbacks=None, verbose=1,
                           save_best_only=True, model_type="tensorflow",
                           hyperparams=None, return_metrics=True,
                           target_col="Close", feature_cols=None,
                           train_on_all=False, **kwargs):
        """
        ...
        version = self.cache_training_data(ticker, timeframe, df)
        ...
        if model_type == "tensorflow":
            ...
            if len(model.input_shape) > 2:  # Sequential data expected
                ...
                if len(X.shape) < 3:
                    ...
                    X_reshaped = np.zeros((X.shape[0] - time_steps + 1, time_steps, n_features))
            ...
            if train_on_all:
                X_train, y_train = X, y
                X_test, y_test = X[split_idx:], y[split_idx:]  # Still need test data for metrics
            else:
    ...
    def wait_for_training(self, thread_id, timeout=None):
        ...
        if not thread.is_alive():
            thread_info["status"] = "completed" if thread_info["error"] is None else "error"
    ...
    def wait_for_all_futures(self, timeout=None):
        ...
        """
        futures = [info["future"] for info in self.futures.values() 
    ...
    def update_model_on_schedule(self, ticker, timeframe, model_id, 
                               schedule="daily", fetch_data_fn=None, 
                               feature_engineering_fn=None, **kwargs):
        """
        ...
        try:
            ...
            self.logger.error("APScheduler is required for scheduling. Install with: pip install apscheduler")
        ...
        self.logger.info(f"Started scheduler for model {model_id} with schedule: {schedule}")
        return scheduler
        
    def monitor_model_performance(self, model_id, ticker, timeframe,
                                 fetch_data_fn=None, feature_engineering_fn=None,
                                 metric="rmse", threshold=0.1, retrain=True,
                                 **retrain_kwargs):
        """
        ...
        try:
            ...
            # Extract features and target
            feature_cols = [col for col in df.columns if col != "Close" and 
                          col != 'date' and col != 'Date']
            X = df[feature_cols].values
            y = df["Close"].values
            ...
            if relative_change > threshold:
                ...
                self.logger.warning(f"Baseline {metric}: {baseline}, Current: {current}")
                self.logger.warning(f"Relative change: {relative_change:.2f} (threshold: {threshold})")
    ...
    def _prune_old_data(self, ticker, timeframe):
        ...
        # Get the cutoff date
        cutoff_date = datetime.datetime.now() - datetime.timedelta(days=self.max_data_age_days)
        
        # Check each version to see if it's older than the cutoff
        for version in versions:
            try:
                # Parse the timestamp from the version
                version_date = datetime.datetime.strptime(version.split("_")[0], "%Y%m%d")
...
def example_usage():
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(64, activation='relu', input_shape=(len(df.columns)-2,)),
    ...
    metrics = learner.monitor_model_performance(
        ...
        ticker=ticker,
        timeframe=timeframe,
        fetch_data_fn=lambda ticker, timeframe: fetch_data(ticker=ticker, start="2023-01-01", interval=timeframe),
