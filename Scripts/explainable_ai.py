...
class XAIExplainer:
    ...
    def __init__(self, model, model_type: str = "tensorflow", feature_names: List[str] = None):
        """
    ...
    def explain_with_shap(self, X: np.ndarray, background_data: Optional[np.ndarray] = None, 
                          plot: bool = True, n_samples: int = 100) -> Any:
        """
            ...
            if 'shap' not in self.explainers:
                if self.model_type == "tensorflow":
                    if background_data is not None:
                        ...
                        self.explainers['shap'] = shap.DeepExplainer(self.model, background_data)
                    else:
                        # Use KernelExplainer as a fallback
                        predict_fn = lambda x: self.predict_wrapper(x)
                        self.explainers['shap'] = shap.KernelExplainer(predict_fn, 
                                                                     shap.sample(X, n_samples))
                    ...
                    if self.feature_names:
                        ...
                        for t in range(timesteps):
                            for f in range(min(n_features, len(self.feature_names))):
                                temporal_feature_names.append(f"{self.feature_names[f]} (t-{timesteps-t})")
                                
                        # Plot for the first instance
                        shap.summary_plot(shap_values[0:1], X_flat[0:1], 
                                         feature_names=temporal_feature_names, show=False)
                    else:
    ...
    def explain_with_lime(self, X: np.ndarray, num_features: int = 10, 
                          num_samples: int = 1000, plot: bool = True) -> Any:
        """
        ...
        try:
            ...
            if self.is_sequential:
                ...
                predict_fn = lambda x: self.predict_wrapper(x.reshape((1,) + X.shape[1:]))[0]
            else:
                predict_fn = lambda x: self.predict_wrapper(x.reshape(1, -1))[0]
            
            # Generate explanation
            explanation = self.explainers['lime'].explain_instance(
                instance_to_explain, predict_fn, num_features=num_features, num_samples=num_samples
            )
    ...
    def explain_with_integrated_gradients(self, X: np.ndarray, 
                                         baseline: Optional[np.ndarray] = None, 
                                         n_steps: int = 50, plot: bool = True) -> Any:
        """
        ...
        try:
            ...
            if self.model_type != "tensorflow":
                raise ValueError("Integrated Gradients is only supported for TensorFlow models")
            ...
            # Generate explanation
            explanation = self.explainers['integrated_gradients'].explain(X, baselines=baseline)
            ...
            if plot:
                ...
                if self.is_sequential and len(attributions.shape) > 2:
                    ...
                    plt.bar(range(avg_attributions.shape[1]), 
                           avg_attributions[0], 
                    ...
                    plt.bar(range(attributions.shape[1]), 
                           attributions[0], 
                           tick_label=self.feature_names if self.feature_names else None)
    ...
    def explain_with_permutation_importance(self, X: np.ndarray, y: np.ndarray, 
                                          n_repeats: int = 10, plot: bool = True) -> Any:
        """
        ...
        try:
            ...
            if plot:
                ...
                plt.bar(range(len(sorted_importances)), sorted_importances, 
                       yerr=sorted_std, tick_label=sorted_names)
    ...
    def explain_with_anchor(self, X: np.ndarray, threshold: float = 0.95, 
                          coverage_samples: int = 1000, plot: bool = True) -> Any:
        """
        ...
        try:
            ...
            if self.is_sequential:
                raise ValueError("Anchor Explanations are not supported for sequential data")
            ...
            if 'anchor' not in self.explainers:
                ...
                self.explainers['anchor'].fit(X, categorical_features=categorical_features)
            
            # We'll explain the first instance
            instance_to_explain = X[0:1]
            
            # Generate explanation
            explanation = self.explainers['anchor'].explain(instance_to_explain, threshold=threshold)
            ...
            if plot:
                ...
                plt.barh(range(len(anchor_rules)), [1] * len(anchor_rules), tick_label=anchor_rules)
                plt.title(f"Anchor Explanation (Precision: {explanation.precision:.2f})")
                plt.tight_layout()
    ...
    def create_partial_dependence_plot(self, X: np.ndarray, feature_idx: int, 
                                     num_points: int = 50, ice: bool = False) -> Any:
        """
        ...
        try:
            ...
            if self.is_sequential:
                raise ValueError("Partial Dependence Plots are not supported for sequential data")
    ...
    def analyze_feature_interactions(self, X: np.ndarray, feature1_idx: int, 
                                   feature2_idx: int, num_points: int = 20) -> Any:
        """
        ...
        try:
            # Not applicable for sequential models
            if self.is_sequential:
                raise ValueError("Feature interaction analysis is not supported for sequential data")
            ...
            # Set plot labels
            feature1_name = self.feature_names[feature1_idx] if self.feature_names else f"Feature {feature1_idx}"
            feature2_name = self.feature_names[feature2_idx] if self.feature_names else f"Feature {feature2_idx}"
    ...
    def get_saliency_map(self, X: np.ndarray) -> Any:
        ...
        try:
            # Only applicable for TensorFlow models
            if self.model_type != "tensorflow":
                raise ValueError("Saliency maps are only supported for TensorFlow models")
    ...
    def create_counterfactual_example(self, X: np.ndarray, target_prediction: float, 
                                    feature_constraints: Dict[int, Tuple[float, float]] = None, 
                                    max_iterations: int = 1000, learning_rate: float = 0.01) -> Any:
        """
        ...
        try:
            # Not applicable for sequential models
            if self.is_sequential:
                raise ValueError("Counterfactual examples are not supported for sequential data")
            ...
            if self.model_type != "tensorflow":
                ...
                raise ValueError("Counterfactual examples are only supported for TensorFlow models")
            ...
            for i in range(max_iterations):
                ...
                if feature_constraints:
                    ...
                    for feat_idx, (min_val, max_val) in feature_constraints.items():
                        X_cf_np[0, feat_idx] = max(min_val, min(max_val, X_cf_np[0, feat_idx]))
            ...
            feature_names = self.feature_names if self.feature_names else [f"Feature {i}" for i in range(len(X[0]))]
            sorted_names = [feature_names[i] for i in indices]
            ...
            plt.bar(x + width/2, counterfactual[0][indices], width, label='Counterfactual')
            
            plt.xlabel('Features')
            plt.ylabel('Value')
            plt.title(f'Counterfactual Example: {original_pred:.2f} → {counterfactual_pred:.2f}')
    ...
    def create_feature_importance_plot(self, feature_importances: Dict[str, float]) -> go.Figure:
        """
    ...
    def create_summary_dashboard(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> Dict[str, Any]:
        """
        ...
        if self.model_type == "tensorflow":
            try:
                dashboard["integrated_gradients"] = self.explain_with_integrated_gradients(X, plot=True)
            except Exception as e:
                logger.warning(f"Could not create Integrated Gradients explanation: {e}")
                dashboard["integrated_gradients"] = {"error": str(e)}
        
        # Add Permutation Importance (if y is provided)
        if y is not None:
            try:
                dashboard["permutation_importance"] = self.explain_with_permutation_importance(X, y, plot=True)
            except Exception as e:
                logger.warning(f"Could not create Permutation Importance explanation: {e}")
        ...
        if self.feature_names and not self.is_sequential:
            try:
                ...
                dashboard["pdp"] = self.create_partial_dependence_plot(X, most_important_feature, ice=True)
            except Exception as e:
...
def explain_model_prediction(model, features, feature_names=None, model_type="tensorflow"):
    """
    ...
    explanations = {
        "prediction": float(predictions[0][0]) if len(predictions.shape) > 1 else float(predictions[0]),
        "features": {name: float(value) for name, value in zip(feature_names, features[0])} if feature_names else features[0].tolist()
    }
...
def get_feature_importance(model, X, feature_names=None, model_type="tensorflow", method="shap"):
    """
    ...
    if feature_names:
        importance = {name: float(imp) for name, imp in zip(feature_names, feature_importance)}
    else:
        importance = {f"Feature_{i}": float(imp) for i, imp in enumerate(feature_importance)}
    
    return importance

def create_xai_component_for_dashboard(model, X_sample, feature_names, predictions=None, actuals=None):
    """
    ...
    try:
        ...
        explanation_data["shap"] = {
            ...
            "values": shap_result["shap_values"] if "shap_values" in shap_result else None
        }
    except Exception as e:
    ...
    if feature_names:
        try:
            # Find top feature
            top_feature = max(explanation_data["feature_importance"].items(), 
                             key=lambda x: x[1])[0]
            top_idx = feature_names.index(top_feature)
            
            pdp_result = explainer.create_partial_dependence_plot(X_sample, top_idx, ice=True)
    ...
    if feature_names and len(feature_names) > 1:
        try:
            # Get top 2 features
            sorted_features = sorted(explanation_data["feature_importance"].items(), 
    ...
    if model_type == "tensorflow" and not explainer.is_sequential:
        try:
            ...
            cf_result = explainer.create_counterfactual_example(X_sample[:1], target_pred)
