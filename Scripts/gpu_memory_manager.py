# Add this to a new file Scripts/gpu_memory_manager.py
...
class GPUMemoryManager:
    ...
    def __init__(self, memory_limit_mb=None, allow_growth=True, 
                 visible_devices=None, reserve_memory=0.1):
        """
    ...
    def estimate_optimal_batch_size(self, model, sample_input_shape, 
                                   min_batch=1, max_batch=8192, 
                                   device_idx=0, safety_factor=0.8):
        """
                ...
                while low <= high:
                    ...
                    try:
                        ...
                        self.clean_memory(force_gc=False)
                            ...
                            tf.errors.UnknownError, tf.errors.OOMError) as e:
...
def train_with_dynamic_batch_size(model, X, y, initial_batch_size=32, min_batch_size=8):
        ...
        try:
            if len(self.logical_gpus) > 1 and use_mirrored:
                ...
                if devices is None:
                    ...
                    self.logger.info(f"Using MirroredStrategy with {len(self.logical_gpus)} GPUs")
                else:
                    # Use specified GPUs
    ...
    def model_memory_profile(self, model, sample_batch_size=1, detailed=False):
        ...
        try:
            # Get input shapes
            input_shapes = [(shape[0] if shape[0] is None else sample_batch_size, *shape[1:])
            ...
            if detailed:
                ...
                self.logger.info(f"{'Layer':<30} {'Output Shape':<20} {'Params':<10} {'Memory (MB)':<15}")
                self.logger.info("-" * 75)
            ...
            for layer in model.layers:
                if hasattr(layer, 'output_shape') and layer.output_shape:
                    ...
                    if isinstance(shape, list):
                        # Multiple outputs
                        layer_act_memory = sum(np.prod([dim if dim is not None else sample_batch_size 
                                              for dim in output_shape]) * 4
                                           for output_shape in shape)
                    else:
                        # Single output
                        layer_act_memory = np.prod([dim if dim is not None else sample_batch_size 
                                                for dim in shape]) * 4
                    ...
                    if detailed:
                        ...
                        self.logger.info(f"{layer.name:<30} {str(shape):<20} {layer_params:<10} {layer_memory_mb:<15.2f}")
            ...
            # Log memory stats
            self.logger.info(f"Model Memory Profile (batch size = {sample_batch_size}):")
            self.logger.info(f"  Parameters: {total_params:,} ({param_memory_mb:.2f} MB)")
            self.logger.info(f"  Gradients: {trainable_params:,} ({gradient_memory_mb:.2f} MB)")
            self.logger.info(f"  Optimizer state: {trainable_params:,} ({optimizer_memory_mb:.2f} MB)")
    ...
    def get_memory_usage_report(self, save_path=None):
        ...
        for device_idx in df['device_idx'].unique():
            ...
            report.append(f"  Memory range: {device_df['memory_mb'].max() - device_df['memory_mb'].min():.2f} MB")
...
def train_with_gradient_accumulation(
    model, train_dataset, steps_per_epoch, accumulation_steps=1, 
    optimizer=None, loss_fn=None, metrics=None, callbacks=None, epochs=1
):
    """
    ...
    for epoch in range(epochs):
        ...
        history['loss'].append(epoch_loss.numpy() if hasattr(epoch_loss, 'numpy') else epoch_loss)
...
def example_usage():
    ...
    optimal_batch = memory_manager.estimate_optimal_batch_size(
        model, sample_input_shape=(28, 28, 1), 
        min_batch=1, max_batch=512
    )
    ...
    print(f"Using actual batch size of {actual_batch} with {accumulation_steps} "
          f"accumulation steps to simulate batch size of {desired_batch}")
    
    # Set up distribution strategy for multi-GPU training
    strategy = memory_manager.create_training_strategy()
