...
from config import RMSE_THRESHOLD, MAPE_THRESHOLD
...
# Define hyperparameter storage directory
HYPERPARAMS_DIR = os.path.join("Data", "Hyperparameters")
...
def create_save_load_interface():
    ...
    with col1:
        ...
        config_name = st.text_input("Configuration Name", 
                                   placeholder="my_hyperparam_config")
        
        # Add optional description
        config_desc = st.text_area("Description (optional)", 
    ...
    with col2:
        ...
        if selected_config:
            ...
            try:
                ...
                st.write(f"**Description**: {config_data.get('description', 'No description')}")
...
def apply_preset_to_ranges(preset_key):
    ...
    for param, config in HYPERPARAMETER_REGISTRY.items():
        if config["type"] in ["int", "float"] and param in st.session_state["original_ranges"]:
            ...
            new_min = max(0, center - new_half_range) if orig_min >= 0 else center - new_half_range
...
def visual_range_editor():
    ...
    for i, (group, params) in enumerate(params_by_group.items()):
        with group_tabs[i]:
            ...
            numerical_params = [(name, config) for name, config in params
            ...
            categorical_params = [(name, config) for name, config in params
...
def visualize_hyperparameter_trends(study_name):
    ...
    try:
        ...
        # Create plots
        fig, ax = plt.subplots(len(selected_params), 1, figsize=(10, 3*len(selected_params)))
        ...
        for i, param in enumerate(selected_params):
            ...
            # Create scatter plot
            sns.scatterplot(x='trial', y=param, data=df, hue='value', 
                           palette='viridis', ax=ax[i])
            
            # Add trend line
            try:
                sns.regplot(x='trial', y=param, data=df, scatter=False, 
...
def create_duration_estimator():
    ...
    with col1:
        ...
        selected_study = st.selectbox(
            "Select a reference study for time estimation",
            studies
        )
        ...
        if avg_time:
            ...
            if total_seconds < 3600:  # Less than 1 hour
                ...
                st.success("✅ Moderate run - good balance between speed and thoroughness")
            else:  # More than 1 day
                st.warning("⚠️ Long run - consider running overnight or using a preset with fewer trials")
        else:
    ...
    with col2:
        ...
        st.write("**Tip**: Start with quick presets for initial exploration, then use thorough presets for final optimization.")
...
def main():
    ...
    section = st.sidebar.radio(
        "Go to",
        ["Current Configuration", "Visual Range Editor", "Presets", 
         "Pruning Settings", "Study Visualization", "Parameter Trends", 
    ...
    if section == "Current Configuration":
        ...
        if studies:
            selected_study = st.selectbox(
                "Select study for parameter trend analysis",
                studies
            )
...
def show_current_config():
    ...
    for group in sorted(set(param["group"] for param in HYPERPARAMETER_REGISTRY.values())):
        with st.expander(f"{group.upper()} Parameters"):
            for name, param in HYPERPARAMETER_REGISTRY.items():
                ...
                if any(model_type in name for model_type in MODEL_TYPES) and \
                   not any(model_type in name for model_type in selected_model_types):
                    continue
                ...
                if param["group"] == group:
                    ...
                    with col2:
                        if param["type"] == "bool":
                            st.checkbox(f"", value=param["default"], key=f"param_{name}", disabled=True)
                        else:
...
def create_pruning_settings():
    ...
    pruning_enabled = st.checkbox("Enable Pruning", 
    ...
    with col1:
        ...
        min_trials = st.number_input("Minimum Completed Trials",
