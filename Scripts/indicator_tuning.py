...
def calculate_werpi_with_params(df: pd.DataFrame, 
                               wavelet_name: str, 
                               level: int,
                               n_states: int, 
...
def evaluate_werpi_performance(df: pd.DataFrame, 
                              werpi_values: np.ndarray,
                              upper_threshold: float = 70,
                              lower_threshold: float = 30,
                              eval_method: str = 'returns') -> float:
    """
    ...
    evaluation_df = pd.DataFrame({
        'close': df['Close'].iloc[-len(werpi_values):].values,
        'werpi': werpi_values
    })
    ...
    evaluation_df.loc[evaluation_df['werpi'] < lower_threshold, 'signal'] = 1  # Buy signal
    evaluation_df.loc[evaluation_df['werpi'] > upper_threshold, 'signal'] = -1  # Sell signal
    
    # Fill forward signals (hold until next signal)
    evaluation_df['signal'] = evaluation_df['signal'].replace(to_replace=0, method='ffill')
    ...
    if eval_method == 'returns':
        ...
        evaluation_df['strategy_return'] = evaluation_df['signal'] * evaluation_df['return']
        ...
        correct_signals = ((evaluation_df['signal'] == 1) & (evaluation_df['return'] > 0)) | \
...
def objective_werpi(trial, df, upper_threshold=70, lower_threshold=30, eval_method='returns'):
    """
    ...
    # Sample parameters
    wavelet_options = [
        'haar', 'db2', 'db4', 'db6', 'sym2', 'sym4', "sym6", "sym8",
          'coif2', 'coif3', 'dmey', "bior1.3", "rbio1.3", 
          "gaus1", "mexh", "morl", "cgau1", "shan", "fbsp", "cmor", "demy"
    ]
    wavelet_name = trial.suggest_categorical('wavelet_name', wavelet_options)
    
    level = trial.suggest_int('level',1,  8)
    n_states = trial.suggest_int('n_states', 1, 8)
    scale_factor = trial.suggest_float('scale_factor', 0.01, 10.0, log=True)
    
    # Calculate WERPI with these parameters
    werpi_values = calculate_werpi_with_params(df, wavelet_name, level, n_states, scale_factor)
...
def optimize_werpi_parameters(df: pd.DataFrame, 
                             n_trials: int = 50, 
                             upper_threshold: float = 70,
                             lower_threshold: float = 30,
    ...
    werpi_values = calculate_werpi_with_params(
        df, best_params['wavelet_name'], best_params['level'], 
        best_params['n_states'], best_params['scale_factor']
    )
...
def calculate_vmli_with_params(df: pd.DataFrame,
                              window_mom: int,
                              window_vol: int,
                              smooth_period: int,
                              winsorize_pct: float,
                              use_ema: bool) -> pd.Series:
    """
...
def evaluate_vmli_performance(df: pd.DataFrame, 
    ...
    evaluation_df.loc[evaluation_df['vmli'] < sell_threshold, 'signal'] = -1  # Sell signal
...
def objective_vmli(trial, df, buy_threshold=0.8, sell_threshold=-0.8, eval_method='returns'):
    """
...
def optimize_vmli_parameters(df: pd.DataFrame, 
                            n_trials: int = 50, 
                            buy_threshold: float = 0.8,
                            sell_threshold: float = -0.8,
                            eval_method: str = 'returns',
...
def optimize_indicators(df: pd.DataFrame, 
...
def tune_and_apply_indicators(df: pd.DataFrame, tune_werpi: bool = True, tune_vmli: bool = True,
                             n_trials: int = 50, eval_method: str = 'returns',
    ...
    from Scripts.features import feature_engineering_with_params
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', choices=['returns', 'correlation', 'signal_quality'], 
                      default='returns', help='Evaluation method')
