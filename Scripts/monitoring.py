        ...
        try:
            if os.path.exists(self.log_file_path):
                ...
                if not loaded_logs.empty:
                    ...
                    logger.info(f"Loaded {len(loaded_logs)} prediction logs from {self.log_file_path}")
                    return True
            return False
    ...
    def save_logs(self) -> bool:
        ...
        try:
            if not self.predictions_log.empty:
                ...
                logger.info(f"Saved {len(self.predictions_log)} prediction logs to {self.log_file_path}")
                return True
            return False
        except Exception as e:
    ...
    def log_prediction(self, ticker: str, timeframe: str, actual: float, predicted: float, horizon: int) -> None:
        ...
        try:
            ...
            if len(self.predictions_log) > self.max_log_entries:
                self.predictions_log = self.predictions_log.iloc[-self.max_log_entries:]
