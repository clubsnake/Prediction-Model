"""
Data Manager for the Prediction Model project.
Handles fetching, processing, and storing financial data.
"""
...
def check_rate_limit(api_name: str) -> bool:
    ...
    with _api_lock:
        ...
        if api_info["count"] >= api_info["limit_per_minute"]:
            ...
            logger.warning(f"{api_name} rate limit reached. Need to wait {seconds_to_wait:.0f} seconds.")
            return False
...
def fetch_ticker_data_finnhub(ticker: str, start_date: str, end_date: Optional[str] = None,
...
def fetch_ticker_data_yfinance(ticker: str, start_date: str, end_date: Optional[str] = None,
    ...
    try:
        logger.info(f"Fetching data from YFinance for {ticker} with interval {interval}")
        if end_date is None:
        ...
        logger.info(f"Successfully fetched {len(df)} records from YFinance for {ticker}")
        return df
        
    except Exception as e:
        logger.error(f"Error fetching data from YFinance for {ticker}: {str(e)}\n{traceback.format_exc()}")
        raise DataFetchError(f"Failed to fetch data from YFinance for {ticker}: {str(e)}")

def fetch_ticker_data(ticker: str, start_date: str, end_date: Optional[str] = None, 
    ...
    while retry_count <= max_retries:
        try:
            ...
            if df is None or df.empty:
                raise DataFetchError(f"Both data sources returned empty data for {ticker}")
            ...
            if retry_count <= max_retries:
                ...
                raise DataFetchError(f"Failed to fetch data after {max_retries+1} attempts")
...
def process_data_job(tickers: List[str] = None, interval: str = "1d", 
                    days_back: int = 365, refresh_rate: int = 3600):
    """
        ...
        try:
            ...
            for i, ticker in enumerate(tickers):
                try:
                    ...
                    # 2. Save raw data
                    raw_file = os.path.join(RAW_DATA_DIR, f"{ticker}_{datetime.now().strftime('%Y%m%d')}_raw.csv")
                    ...
                    if i < len(tickers) - 1:
                        ...
                        logger.debug(f"Waiting {delay}s before next ticker to respect API limits")
            ...
            logger.error(f"Error in data processor main loop: {str(e)}\n{traceback.format_exc()}")
...
def get_latest_processed_data(ticker: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        files = [f for f in os.listdir(PROCESSED_DATA_DIR) 
                if f.startswith(ticker) and f.endswith('.csv') and not f.endswith('_meta.csv')]
        ...
        logger.info(f"Loaded latest data for {ticker} from {os.path.basename(latest_file)}")
...
def fetch_latest_data_for_prediction(ticker: str, days: int = 60) -> Optional[pd.DataFrame]:
    """
