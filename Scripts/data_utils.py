...
from Scripts.vectorized_ops import vectorized_sequence_creation, batch_sequence_creation
from Scripts.robust_handler import error_boundary
...
def load_and_prepare_data(file_path: str, 
    ...
    try:
        ...
        if standardize:
            ...
            train_df_scaled = pd.DataFrame(train_features, columns=feature_cols, index=train_df.index)
            train_df_scaled[target_col] = train_target
            test_df_scaled = pd.DataFrame(test_features, columns=feature_cols, index=test_df.index)
